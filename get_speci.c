...
static void			applydi(t_data *arginp, intmax_t inp)
{
	...
	arginp->nb += ft_putnchar('0', arginp->prec - ft_numlen(inp, 10) + (inp < 0));
	if (arginp->canprint)
		arginp->nb += ft_putnbrl(ft_abs(inp));
	if (arginp->flagmin)
		arginp->nb += ft_putnchar(' ', arginp->wid - ft_max(arginp->prec, ft_numlen(inp, 10)) - (arginp->flagplu && inp >= 0));
}
...
static void			applyuo(t_data *arginp, int base, uintmax_t inp)
{
	if (!(!arginp->flaghas && inp == 0 && arginp->prec == 0) || arginp->isprec == 0)
		arginp->canprint = 1;
	if (!arginp->flagmin && !arginp->flagzer)
		arginp->nb += ft_putnchar(' ', arginp->wid - ft_max(arginp->prec, (arginp->canprint || base == 10) * ft_numlen(inp, base)) - arginp->flaghas);
	if (arginp->flaghas && inp != 0)
		arginp->nb += ft_putstr("0");
	if (!arginp->flagmin && arginp->flagzer)
		arginp->nb += ft_putnchar('0', arginp->wid - ft_max(arginp->prec, (arginp->canprint || base == 10) * ft_numlen(inp, base)) - arginp->flaghas);
	...
	if (arginp->flagmin)
		arginp->nb += ft_putnchar(' ', arginp->wid - ft_max(arginp->prec, ft_numlen(inp, base)) - arginp->flaghas);
}
...
static void			applyxx(t_data *arginp, char c, uintmax_t inp)
{
	if (!(inp == 0 && arginp->prec == 0) || arginp->isprec == 0)
		arginp->canprint = 1;
	if (!arginp->flagmin && !arginp->flagzer)
		arginp->nb += ft_putnchar(' ', arginp->wid - ft_max(arginp->prec, arginp->canprint * ft_numlen(inp, 16)) - arginp->flaghas * 2);
	if (c == 'x' && arginp->flaghas && inp != 0)
		arginp->nb += ft_putstr("0x");
	else if (c == 'X' && arginp->flaghas && inp != 0)
		arginp->nb += ft_putstr("0X");
	if (!arginp->flagmin && arginp->flagzer)
		arginp->nb += ft_putnchar('0', arginp->wid - ft_max(arginp->prec, arginp->canprint * ft_numlen(inp, 16)) - arginp->flaghas * 2 - arginp->flagsp);
	...
	if (arginp->flagmin)
		arginp->nb += ft_putnchar(' ', arginp->wid - ft_max(arginp->prec, ft_numlen(inp, 16)) - arginp->flaghas * 2);
}
